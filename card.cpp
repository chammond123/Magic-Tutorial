...
QImage Card::getImage() const
{
    return image;
}
ManaType Card::getColor() const
{
    return color;
}
...
QVector<ManaType> Card::getCost() const
{
    return cost;
}
QVector<PropertyType> Card::getKeywords() const
{
    return keywords;
}
...
{
    return (type == CardType::CREATURE || type == CardType::ARTIFACT
            || type == CardType::ENCHANTMENT || type == CardType::PLANESWALKER
            || type == CardType::ENCHANTMENT || type == CardType::LAND);
}
